...
- A store has one primary domain (`is_primary`, read through `Store::primaryDomain()`); any other active domains are aliases that also resolve the store. The first domain `CreateStoreAction` creates is the primary for the life of the store: nothing replaces, demotes or removes it, and the database allows only one live primary per store.
- Add aliases through `Actions\AddStoreDomainAliasAction` (its caller normalizes and validates the domain) and remove one through `Actions\RemoveStoreDomainAliasAction` (it keeps the alias as trashed history and refuses the primary); both force a storefront redeploy, because the container's Traefik rule lists every active domain and labels cannot change in place.
...
- `CreateStorePage` skips the request only for explicit `create_storefront=false`. The console form exposes this through `StorefrontConfigurationFields::creationToggle()`; the reseller form remains mandatory. Both collect only image and slug and deploy with sample storefront details. The tenant administrator replaces contact and business details on the admin Storefront settings page.
