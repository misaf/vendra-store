...
it('queues replacement provisioning attempts while preventing concurrent processing', function (): void {
    ...
    dispatch(new CompleteStoreProvisioningJob(123));
    dispatch(new CompleteStoreProvisioningJob(123));
    ...
    expect($job->middleware())
        ->toHaveCount(1)
        ->and(Arr::get($job->middleware(), 0))->toBeInstanceOf(WithoutOverlapping::class);
});
...
it('completes provisioning checkpoints before activating the tenant', function (): void {
    ...
    app()->call([new CompleteStoreProvisioningJob($tenant->id), 'handle']);
    ...
    Event::assertDispatched(fn (TenantProvisioned $event): bool => $event->tenant->is($tenant) && $event->shouldSeed);
...
it('records a failed checkpoint and safely retries unfinished work', function (): void {
    ...
    expect(fn () => app()->call($job->handle(...)))->toThrow(RuntimeException::class, 'Seeder failed.')
        ->and($tenant->refresh()->active)->toBeFalse()
...
it('scopes provisioning identifiers without leaking them afterward', function (): void {
    ...
    expect($captured)->not->toBeEmpty()
        ->and(Arr::get($captured, 0)->traceId)->toBeUuid()
        ->and(Arr::get($captured, 0)->operation)->toBe('store_provisioning')
        ->and(Arr::get($captured, 0)->tenantId)->toBe($tenant->getKey())
        ->and(Arr::get($captured, 0)->metadata[ContextKeys::RESELLER_ID])->toBe($reseller->getKey())
        ->and(Context::get(RequestJobContext::OPERATION))->toBe('outer')
        ->and(Context::has(RequestJobContext::TENANT_ID))->toBeFalse();
});

it('marks an unfinished store failed but never a ready one when the job gives up', function (): void {
    $pending = Store::factory()->create([
        'active' => false,
        'provisioning_status' => TenantProvisioningStatus::Processing,
    ]);
    $ready = Store::factory()->create([
        'active' => true,
        'provisioning_status' => TenantProvisioningStatus::Ready,
    ]);

    new CompleteStoreProvisioningJob($pending->id)->failed(null);
    new CompleteStoreProvisioningJob($ready->id)->failed(new RuntimeException('Late failure.'));

    expect($pending->refresh()->provisioning_status)->toBe(TenantProvisioningStatus::Failed)
        ->and($pending->provisioning_error)->toBe('Store provisioning failed.')
        ->and($pending->provisioning_failed_at)->not->toBeNull()
        ->and($ready->refresh()->provisioning_status)->toBe(TenantProvisioningStatus::Ready)
        ->and($ready->active)->toBeTrue()
        ->and($ready->provisioning_error)->toBeNull();
});
